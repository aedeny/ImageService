            ...
            function(_Popover) {
                ...
                _proto._setConfirmationListeners = function _setConfirmationListeners() {
                    ...
                    if (!this.config.selector) {
                        ...
                        $(this.element).on(this.config.trigger,
                            function(e, ack) {
                                if (!ack) {
                                    e.preventDefault();
                                    e.stopPropagation();
                ...
                _proto._setStandardButtons = function _setStandardButtons($tip) {
                    var self = this;
                    var btnApply = $tip.find(Selector.BTN_APPLY).addClass(this.config.btnOkClass)
                        .html(this.config.btnOkLabel).attr(this.config._attributes);

                    if (this.config.btnOkIconClass || this.config.btnOkIconContent) {
                        btnApply.prepend($('<i></i>').addClass(this.config.btnOkIconClass || '')
                            .text(this.config.btnOkIconContent || ''));
                    }

                    btnApply.off('click').one('click',
                        function(e) {
                            if ($(this).attr('href') === '#') {
                                e.preventDefault();
                            }
                    ...
                    var btnDismiss = $tip.find(Selector.BTN_DISMISS).addClass(this.config.btnCancelClass)
                        .html(this.config.btnCancelLabel);

                    if (this.config.btnCancelIconClass || this.config.btnCancelIconContent) {
                        btnDismiss.prepend($('<i></i>').addClass(this.config.btnCancelIconClass || '')
                            .text(this.config.btnCancelIconContent || ''));
                    }

                    btnDismiss.off('click').one('click',
                        function(e) {
                            e.preventDefault();
                ...
                _proto._setCustomButtons = function _setCustomButtons($tip) {
                    ...
                    this.config.buttons.forEach(function(button) {
                        var btn = $('<a href="#"></a>').addClass(BTN_CLASS_DEFAULT)
                            .addClass(button.class || 'btn btn-secondary').html(button.label || '')
                            .attr(button.attr || {});
                ...
                Confirmation._jQueryInterface = function _jQueryInterface(config) {
                    return this.each(function() {
                        ...
                        _config.rootSelector =
                            $(this).selector || _config.rootSelector; // this.selector removed in jQuery > 3
